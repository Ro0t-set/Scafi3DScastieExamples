package examples.sphere_channel
...
final case class EngineImpl(ncols: Int, nrows: Int, ndepth: Int)(
  ...
  net.chgSensorValue("source", Set(1), true)

  net.addSensor("target", false)
  net.chgSensorValue("target", Set(50), true)
  ...
  override def executeIterations(): Unit =
    ...
    val theta   = Math.PI * (1 + Math.sqrt(5)) * id
    ...
    net.exec(ChannelProgram, ChannelProgram.main(), nextIdToRun)
